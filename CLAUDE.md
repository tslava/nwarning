...
`StorageMonitor` keeps the value each key held before _it_ first changed it and
...
`src/shared/images/icon_*.png` are generated by `tools/icon.py`; the PNGs are
committed but editing one by hand is pointless, since the next run of the script
overwrites it. The script uses the banner's colours by definition, so the icon
cannot drift from what the banner shows.

It draws the halves and the gap at each target size on whole pixels, taking only
the circular edge from a supersampled mask. Rendering the whole thing large and
scaling down puts the gap's edges inside pixels, and at 16px the 2px white gap
then resolves as two grey columns — measurably, not as a matter of taste.
